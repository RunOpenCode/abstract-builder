node {
    stage ('Provisioning') {
        git 'https://github.com/RunOpenCode/abstract-builder'
    }
    ...
    stage('SonarQube') {
        ...
        withSonarQubeEnv {
            sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=runopencode-abstract-builder -Dsonar.projectName='Abstract builder' -Dsonar.projectVersion=1.0 -Dsonar.sources=src -Dsonar.language=php -Dsonar.sourceEncoding=UTF-8 -Dsonar.tests=test -Dsonar.php.tests.reportPath=build/logs/junit.xml -Dsonar.php.coverage.reportPath=build/logs/clover.xml -Dsonar.clover.reportPath=build/logs/clover.xml -Dsonar.coverage.exclusions=test"
        }
    }
    stage('Reporting') {
